(function(){
  ...
  function renderTower(){$('tower').innerHTML=state.floors.map((floor,index)=>{const room=ROOMS[floor.type],people=(floor.residents||[]).map(residentById).filter(Boolean);return`<article class="floor room-${floor.type}"><div class="floor-head"><b>F${index+1} · ${room.name}</b><span>${room.category}${floor.type==='dorm'?` · ${people.length}/6 人`:''}</span></div><div class="floor-body">${people.length?people.map(person=>`<button class="person sprite-row-${Math.floor(person.sprite/4)}" data-person="${person.id}" style="background-position:${spritePosition(person.sprite)}" aria-label="查看 ${person.name}"></button>`).join(''):'<span class="floor-empty">空房间</span>'}</div></article>`}).join('');document.querySelectorAll('[data-person]').forEach(button=>button.onclick=()=>openPerson(button.dataset.person))}
