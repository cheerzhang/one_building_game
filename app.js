(function(){
  ...
  function renderTower(){$('tower').innerHTML=state.floors.map((floor,index)=>{const room=ROOMS[floor.type],people=(floor.residents||[]).map(residentById).filter(Boolean);return`<article class="floor"><div class="floor-head"><b>F${index+1} · ${room.name}</b><span>${room.category}${floor.type==='dorm'?` · ${people.length}/6 人`:''}</span></div><div class="floor-body">${people.length?people.map(person=>`<button class="person" data-person="${person.id}" style="background-position:${spritePosition(person.sprite)}" aria-label="查看 ${person.name}"></button>`).join(''):'<span class="floor-empty">空房间</span>'}<span class="room-symbol">${room.icon}</span></div></article>`}).join('');document.querySelectorAll('[data-person]').forEach(button=>button.onclick=()=>openPerson(button.dataset.person))}
  function renderCatalog(){$('roomCatalog').innerHTML=Object.entries(ROOMS).map(([type,room])=>`<article class="room-card"><div class="icon">${room.icon}</div><div><h3>${room.name} · ${room.category}</h3><p>${room.description}</p></div><button type="button" data-build="${type}" ${state.money<room.cost?'disabled':''}>${euro(room.cost)}</button></article>`).join('');document.querySelectorAll('[data-build]').forEach(button=>button.onclick=()=>buildRoom(button.dataset.build))}
