...
function validPolicy(policy){const network=policy?.best?.policy;return Boolean(policy&&policy.version===2&&policy.trainingVersion===23&&network?.format==='action-network-v1'&&network.features===24&&network.hidden===16&&Array.isArray(network.actions)&&network.actions.length===39&&Array.isArray(network.weights)&&network.weights.length===1063&&network.weights.every(Number.isFinite))}
