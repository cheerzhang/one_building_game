...
function validPolicy(policy){const network=policy?.best?.policy;return Boolean(policy&&policy.version===2&&policy.trainingVersion===31&&network?.format==='action-network-v1'&&network.features===48&&network.hidden===16&&Array.isArray(network.actions)&&network.actions.length===39&&Array.isArray(network.weights)&&network.weights.length===1447&&network.weights.every(Number.isFinite))}
